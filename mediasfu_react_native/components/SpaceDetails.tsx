...
const SpaceDetails: React.FC = () => {
  ...
  const allRoomAudioStreams = useRef<(Participant | Stream)[]>([]);
  ...
  // const mainVideoStream = useRef<Participant | Stream | null>(null);
  ...
  const createRoom = () => {
    ...
    showRoomDetails.current = {
      action: 'create',
      duration: (space?.duration || 1) / (60 * 1000), // Convert to minutes
  ...
  // Handle source parameters updates
  useFocusEffect(
    React.useCallback(() => {
      if (
        ...
        if (noRoom) {
          if (sourceParameters.current.roomName !== '') {
            ...
            // To avoid rendering of autoWave
        ...
        if (sourceParameters.current.audioLevel) {
        ...
        if (
          sourceParameters.current.allAudioStreams !==
          allRoomAudioStreams.current
        ) {
          allRoomAudioStreams.current =
            sourceParameters.current.audioOnlyStreams;
        }
        if (
        ...
        if (sourceParameters.current.alertMessage !== mediasfuAlert) {
    ...
    }, [space, showRoom, videoOn, isMuted, mediasfuAlert, isConnected, sourceChanged]),
  );
  ...
  const handleSelectCamera = async (deviceId: string) => {
    ...
    try {
      ...
      setMessage('Error selecting camera.');
      console.error(error);
    }
  };
      ...
      <View style={styles.participantsSection}>
        ...
        <FlatList
          ...
          renderItem={({item}) => {
            ...
            // option 2, look in the participants array for the video stream with the name as the participant id
            // try {
            //   const refParticipant = sourceParameters?.current
            //     ?.getUpdatedAllParams()
            //     .participants?.find((part: Participant) => part.name === p.id);
            ...
            // option 1, look in allRoomVideos (need to flatten it) for the video stream with the name as the participant id
            try {
              const video_stream = allRoomVideos.current
                ...
                .find(
                  stream =>
                    ...
                    (stream.props.name.includes('youyou') &&
                      p.id === currentUser?.id),
                );
